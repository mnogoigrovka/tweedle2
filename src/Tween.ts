...
export class Tween<Target> {
	 ...
	 * @returns returns this tween for daisy chaining methods.
	 */
	public start(delay?: number, overwrite?: boolean): this {
		if (this._isPlaying) {
			return this;
		}
	...
	private _internalUpdate(deltaTime: number): boolean {
		...
		// нахуй никому не нужно выполнение твина, когда он стоит на делее
		if (currentTime < 0) {
			return true;
		}
		...
		if (this._onAfterDelayCallbackFired == false && currentTime >= 0) {
			...
			if (this._overwrite) {
				this._setupProperties(this._object, this._valuesStart, this._valuesEnd, this._valuesStartRepeat, true);
			}
		}
